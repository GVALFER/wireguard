...
- 📄 **Simple config files** - Direct access to WireGuard configurations
...
**Scan QR code or copy the configuration file directly!**
...
```bash
Server endpoint (default: your-server.com:51820): vpn.company.com:51820
...
sudo ./wg-manage.sh config

# View active peers
sudo ./wg-manage.sh peers
```
...
### Copy Configuration to Client Device

After creating a client, you can:

1. **Copy config file directly:**
   ```bash
   # From server
   sudo cat /etc/wireguard/clients/laptop.conf
   
   # Copy output to your WireGuard client
   ```

2. **Use QR code** (displayed automatically)
...
3. **Transfer file via SCP:**
   ```bash
   sudo scp /etc/wireguard/clients/laptop.conf user@client-machine:/path/to/save/
   ```
